...
func (h *httpWatcher) getHTTP() (uint32, bool, error) {
	...
	if err != nil {
		return 0, true, errors.Wrap(err, fmt.Sprintf("can not get url (%v) (%v)", url, err))
	}
	...
	if h.useRegexp {
		...
		if err != nil {
			return 0, true, errors.Wrap(err, fmt.Sprintf("can not read body (%v) (%v)", url, err))
		}
